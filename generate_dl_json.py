...
def add_field_info(
    ...
    if (not pd.isnull(group)) and (group != "") and (group is not np.NaN):
        temp = {
            ...
            "bit_end" : str(bit_end),
            "type" : str(datatype),
            "coefficient" : str(filter_nan(coefficient))
        }        
...
csv_path = os.path.join(dir_name, "resources\\DL\\AC_OutletFieldsDL.csv")
json_path = os.path.join(dir_name, "DL_AC_Outlet.json")
#################################################################################################################################
